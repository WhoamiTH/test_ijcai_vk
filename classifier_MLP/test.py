...
def get_reference_data(train_data, positive_data, negative_data, informative_minority_data, border_majority_data,  ref_data_type):
    ...
    if ref_data_type == 'im':
        return informative_minority_data

    if ref_data_type == 'bm':
        return border_majority_data

    if ref_data_type == 'both':
        return np.vstack((informative_minority_data, border_majority_data))
...
ref_data_type, k_iteration, ref_num_type, ref_times, boundary_type = test_method.split('_')
...
for i in range(k_iteration):
    ...
    im, bm = get_infor_data(kn_data, kn_label)
