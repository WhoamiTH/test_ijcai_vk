...
def generate_train_data_label(pre_data, pos_data, target_label):
...
def transform_data_to_train_form(infor_method, train_data_Pos, train_data_neg, infor_Pos, border_neg):
    if infor_method == 'normal':
        neg_neg_data_label = generate_train_data_label(train_data_neg, border_neg, np.array([0]))
        neg_pos_data_label = generate_train_data_label(train_data_neg, train_data_Pos, np.array([0]))
        pos_neg_data_label = generate_train_data_label(train_data_Pos, train_data_neg, np.array([1]))
        pos_pos_data_label = generate_train_data_label(train_data_Pos, train_data_Pos, np.array([1]))
    ...
    if infor_method == 'im':
        ...
        neg_neg_data_label = generate_train_data_label(train_data_neg, border_neg, np.array([0]))
        neg_pos_data_label = generate_train_data_label(train_data_neg, infor_Pos, np.array([0]))
        pos_neg_data_label = generate_train_data_label(infor_Pos, train_data_neg, np.array([1]))
        pos_pos_data_label = generate_train_data_label(infor_Pos, infor_Pos, np.array([1]))
    ...
    if infor_method == 'im2':
        ...
        neg_pos_data_label = generate_train_data_label(train_data_neg, train_data_Pos, np.array([0]))
        pos_neg_data_label = generate_train_data_label(train_data_Pos, train_data_neg, np.array([1]))
        pos_pos_data_label = generate_train_data_label(train_data_Pos, infor_Pos, np.array([1]))
    ...
    if infor_method == 'both3':
        ...
        neg_neg_data_label = generate_train_data_label(train_data_neg, border_neg, np.array([0]))
        neg_pos_data_label = generate_train_data_label(train_data_neg, infor_Pos, np.array([0]))
        pos_neg_data_label = generate_train_data_label(train_data_Pos, border_neg, np.array([1]))
        pos_pos_data_label = generate_train_data_label(train_data_Pos, infor_Pos, np.array([1]))
...
model_type, infor_method, k_iteration, num_epochs = train_method.split('_')
num_epochs = int(num_epochs)
k_iteration = int(k_iteration)
...
for i in range(k_iteration):
    im_label = np.ones((im.shape[0], 1))
    bm_label = np.zeros((bm.shape[0], 1))

    kn_data = np.vstack((im, bm))
    kn_label = np.vstack((im_label, bm_label))

    im, bm = get_infor_data(infor_method, kn_data, kn_label, im, bm)

informative_minority_data = im
border_majority_data = bm
...
for train_epoch in dependency_list:
    ...
    history_train_method = 'MLP_{0}_{1}_{2}'.format(infor_method, k_iteration, train_epoch)
