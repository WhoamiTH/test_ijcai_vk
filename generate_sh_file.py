        ...
        for k_iteration in k_list:
            ...
            for dataset_index in range(1, 6):
                for item_early_stop_type in early_stop_type_list:
                    cur_train_method = 'MLP_{0}_{1}_{2}'.format(sample_method, k_iteration, early_stop_type)
...
for file_index in range(1, total_file_num+1):
    ...
    with open('job_{0}.qjob'.format(file_index), 'w') as fsh:
        ...
        fsh.write('#PBS -m ae\n')
        fsh.write('#PBS -j oe\n\n')
        fsh.write('#PBS -o /srv/scratch/z5102138/test_ijcai_vk/\n')
        fsh.write('source ~/anaconda3/etc/profile.d/conda.sh\n')
        fsh.write('conda activate py36\n\n\n')
        fsh.write('cd /srv/scratch/z5102138/test_ijcai_vk\n')
